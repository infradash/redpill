...
func TestStorageOrchestrate(t *testing.T) { TestingT(t) }

type StorageOrchestrateTests struct {
}

var _ = Suite(&StorageOrchestrateTests{})

const (
	testdb = "test_orchestrate.db"
)

func (suite *StorageOrchestrateTests) SetUpSuite(c *C) {
...
func (suite *StorageOrchestrateTests) TestInitDb(c *C) {
	db, err := init_orchestrate_db(os.TempDir(), testdb)
	defer db.Close()
	c.Assert(err, Equals, nil)
	c.Log(db)
...
func (suite *StorageOrchestrateTests) TestOrchestrateModelStorageOrchestrate(c *C) {
	db, err := init_orchestrate_db(os.TempDir(), testdb)
	defer db.Close()
	c.Assert(err, Equals, nil)
	c.Log(db)
...
func (suite *StorageOrchestrateTests) TestOrchestrateInstanceStorageOrchestrate(c *C) {
	db, err := init_orchestrate_db(os.TempDir(), testdb)
	defer db.Close()
	c.Assert(err, Equals, nil)
	c.Log(db)
